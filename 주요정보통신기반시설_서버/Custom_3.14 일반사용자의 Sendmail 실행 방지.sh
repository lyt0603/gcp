...
if [ -n "$sendmail_process" ]; then
    ...
    if [ -f "$sendmail_cf" ]; then
        ...
        if [ -n "$privacy_options" ]; then
            # 주석을 제거하고 PrivacyOptions 값을 확인
            uncommented_options=$(echo "$privacy_options" | grep -vE '^[[:space:]]*#')

            # restrictqrun 옵션 확인
            if echo "$uncommented_options" | grep -qiE "PrivacyOptions=.*restrictqrun"; then
                echo "PrivacyOptions 설정에 restrictqrun 옵션이 포함되어 있습니다."
            else
                echo "PrivacyOptions 설정에 restrictqrun 옵션이 포함되지 않았거나 주석 처리되어 있습니다."
                echo "----------------------------------------"
...
if [ $result -eq 0 ]; then
    ...
    echo "/etc/mail/sendmail.cf 파일에서 PrivacyOptions 설정을 확인해주세요."
    echo "필요 옵션 : restrictqrun"
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 3.14 일반사용자의 Sendmail 실행 방지"
