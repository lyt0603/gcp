...
check_file() {
    ...
    if [ "$special" -ne 0 ] || [ "$owner" -gt 6 ] || [ "$group" -gt 0 ] || [ "$other" -gt 0 ]; then
...
# 모든 사용자 홈 디렉터리 및 /etc/hosts.equiv 파일 목록
files="/etc/hosts.equiv"
files="$files $(awk -F: '{if ($6 ~ /^\/(home|root)/) print $6"/.rhosts"}' /etc/passwd | tr -d ' ')"
...
for file in $files; do
    ...
    if grep -q '+' "$file" 2>/dev/null; then
        if grep -Eq '^\s*#.*\+' "$file" 2>/dev/null; then
            echo "$file: '+'는 주석처리 되어 있습니다."
        else
            echo "$file: 파일 내용에 '+'가 포함되어 있습니다."
            result=1
        fi
    fi
done
...
if [ $result -eq 0 ]; then
    ...
    echo "$HOME/.rhosts, /etc/hosts.equiv 파일의 소유자, 권한 및 내용을 확인해주세요."
    echo "파일 내용에 '+'가 포함되었는지 확인하고 수정해주세요."
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 2.13 /etc/hosts.equiv 및 모든 사용자 .rhosts 사용 금지"
