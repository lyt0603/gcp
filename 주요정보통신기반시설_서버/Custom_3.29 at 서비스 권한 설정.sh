...
# [1] /usr/bin/at 또는 /bin/at 파일 검사
at_paths="/usr/bin/at"
for at_file in $at_paths; do
    if [ -f "$at_file" ]; then
        echo "파일 경로: $at_file"

        # 파일 권한 확인
        file_permissions=$(ls -ld "$at_file" | awk '{print $1}')
        ...
        # 조건 확인: 특수권한 없음, owner <= 7, group <= 5, other == 0
        if [ "$special" -ne 0 ] || [ "$owner" -gt 7 ] || [ "$group" -gt 5 ] || [ "$other" -gt 0 ]; then
            echo "$at_file: 권한이 잘못 설정되었습니다. 권한: $file_permissions, numeric_permissions: $numeric_permissions"
            result=1
        fi
    fi
done

# [2] at.allow 또는 at.deny 파일 검사
cron_paths="/etc/cron.d /etc /var/adm/cron"
cron_files="at.allow at.deny"
for path in $cron_paths; do
    for file in $cron_files; do
        ...
        if [ -f "$full_path" ]; then
            ...
            # 파일 소유자 및 권한 확인
            file_owner=$(ls -ld "$full_path" | awk '{print $3}')
            file_permissions=$(ls -ld "$full_path" | awk '{print $1}')
            numeric_permissions=$(calculate_permissions "$file_permissions")
            ...
            if [ "$file_owner" != "root" ]; then
                echo "$full_path: 소유자가 root가 아닙니다. 소유자: $file_owner"
                result=1
            fi

            # 조건 확인: 특수권한 없음, owner <= 6, group <= 4, other == 0
            if [ "$special" -ne 0 ] || [ "$owner" -gt 6 ] || [ "$group" -gt 4 ] || [ "$other" -gt 0 ]; then
                echo "$full_path: 권한이 잘못 설정되었습니다. 권한: $file_permissions, numeric_permissions: $numeric_permissions"
                result=1
            fi
        fi
    done
...
if [ $result -eq 0 ]; then
    ...
    echo "/usr/bin/at 및 at.allow 또는 at.deny 관련 파일의 소유자 및 권한을 확인하고 조치를 취해주세요."
    echo "확인이 필요한 파일: /usr/bin/at 권한 750 이하, at.allow 또는 at.deny 소유자 root, 권한 640 이하"
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 3.29 at 서비스 권한 설정"
