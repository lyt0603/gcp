...
check_file() {
    ...
    if [ "$special" -eq 0 ] && [ "$owner" -le 6 ] && [ "$group" -le 4 ] && [ "$other" -le 0 ]; then
        if [ "$file_owner" = "root" ]; then
            echo "$file: 조건이 충족되었습니다."
            return 0
        else
            echo "$file: 소유자가 root가 아닙니다. 소유자: $file_owner, 권한: $file_permissions ($numeric_permissions)"
            return 1
        fi
    else
        echo "$file: 권한이 잘못 설정되었습니다. 소유자: $file_owner, 권한: $file_permissions ($numeric_permissions)"
        return 1
    fi
}

# 확인할 파일 목록
files="/etc/ftpusers /etc/ftpd/ftpusers /etc/vsftpd/ftpusers /etc/vsftpd/user_list /etc/vsftpd.ftpusers /etc/vsftpd.user_list"
...
if [ $result -eq 0 ]; then
    ...
    echo "ftpusers 파일의 소유자와 권한을 확인하고 조치를 취해주세요."
    echo "확인이 필요한 파일: /etc/ftpusers, /etc/ftpd/ftpusers, /etc/vsftpd/ftpusers, /etc/vsftpd/user_list, /etc/vsftpd.ftpusers, /etc/vsftpd.user_list"
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 3.27 ftpusers 파일 소유자 및 권한 설정"
