...
check_file() {
    ...
    if [ "$file_owner" != "root" ] && [ "$file_owner" != "bin" ] && [ "$file_owner" != "sys" ]; then
        echo "$file: 소유자가 root, bin, sys 중 하나가 아닙니다. 소유자: $file_owner"
        error_flag=1
    fi
...
for file in $files; do
    if ! check_file "$file"; then
        result=1  # 조건이 충족되지 않은 파일이 존재
    fi
done

# 최종 결과 반환
if [ $result -eq 0 ]; then
    echo "모든 파일이 조건을 만족합니다. exit 0"
    exit 0
else
    echo "조건이 확인되지 않았습니다. exit 1"
    echo "/etc/syslog.conf /etc/rsyslog.conf 권한이 640 이하가 아니거나, 소유자가 root, bin, sys가 아닙니다."
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 2.7 syslog.conf 파일 소유자 및 권한 설정"
