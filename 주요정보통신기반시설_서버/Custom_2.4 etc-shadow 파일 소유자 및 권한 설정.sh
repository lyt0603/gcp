...
check_file() {
    ...
    # 조건 확인: 특수권한 없음, owner <= 4, group == 0, other == 0
    if [ "$special" -eq 0 ] && [ "$owner" -le 4 ] && [ "$group" -eq 0 ] && [ "$other" -eq 0 ]; then
        echo "$file: 조건이 충족되었습니다."
        return 0
    else
        echo "$file: 권한이 잘못 설정되었습니다. 권한: $file_permissions ($numeric_permissions)"
        result=1  # 조건 불충족 시 결과를 1로 설정
        return 1
    fi
}

# 검사 대상 파일들
files_to_check="/etc/shadow /etc/security/passwd /tcb/files/auth"
...
if [ $result -eq 0 ]; then
    ...
    echo "각 파일의 권한이 400 이하가 아니거나, 소유자가 root가 아닙니다."
    echo "확인이 필요한 파일: /etc/shadow /etc/security/passwd /tcb/files/auth"
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 2.4 shadow 파일 소유자 및 권한 설정"
