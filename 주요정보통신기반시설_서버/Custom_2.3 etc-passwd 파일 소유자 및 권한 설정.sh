...
if [ -f "$passwd_file" ]; then
    echo "파일 경로: $passwd_file"

    # 파일 소유자 확인
    file_owner=$(ls -ld "$passwd_file" | awk '{print $3}')
    if [ "$file_owner" != "root" ]; then
        echo "$passwd_file: 소유자가 root가 아닙니다. 소유자: $file_owner"
        result=1
    fi

    # 파일 권한 확인
    file_permissions=$(ls -ld "$passwd_file" | awk '{print $1}')
    ...
    if [ "$special" -ne 0 ] || [ "$owner" -gt 6 ] || [ "$group" -gt 4 ] || [ "$other" -gt 4 ]; then
        echo "$passwd_file: 권한이 잘못 설정되었습니다. 권한: $file_permissions ($numeric_permissions)"
        result=1
    fi
else
    echo "$passwd_file: 파일이 존재하지 않습니다."
    result=1
fi
...
if [ $result -eq 0 ]; then
    ...
    echo "/etc/passwd 권한이 644 이하가 아니거나, 소유자가 root가 아닙니다."
    echo "주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드 - 2.3 passwd 파일 소유자 및 권한 설정"
